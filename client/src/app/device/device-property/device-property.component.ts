...
export class DevicePropertyComponent implements OnInit, OnDestroy {
	...
	getSecurity(): any {
		if (this.data.device.type !== DeviceType.OPCUA || !this.propertyExpanded) {
			...
			if (this.securityRadio || this.security.username || this.security.password || this.security.accesskey || this.security.sn) {
				let result = { mode: this.securityRadio, uid: this.security.username, pwd: this.security.password, accesskey: this.security.accesskey, sn: this.security.sn };
				return result;
			} else {
				return null;
			}
	...
	setSecurity(security: string) {
		if (security && security !== 'null') {
			...
			this.security.sn = value.sn;
			this.security.accesskey = value.accesskey;
